...
def build_afni_proc_call(config):
    ...
    # Builds an afni command.  An example of a command generated by this process could be:
    # afni_proc.py -subj_id 509 -blocks tshift align tlrc volreg mask combine    \
    #     blur scale regress -copy_anat skull_strip_out.nii -anat_has_skull no   \
    #     -dsets_me_run BOLD_e1.nii.gz BOLD_e2.nii.gz BOLD_e3.nii.gz             \
    #     -echo_times ECHO_TIME_1 ECHO_TIME_2 ECHO_TIME_3                        \
    #     -reg_echo ECHO_NUM_FOR_REGISTRATION                                    \
    #     -tcat_remove_first_trs NUM_TRS_TO_REMOVE                               \
    #     -align_opts_aea -cost ALIGNMENT_COST_FUNCTION                          \
    #     -tlrc_base STANDARD_TEMPLATE -tlrc_NL_warp                             \
    ...
    # To decode this, see the help for afni_proc.py:
...
def run_afni_command(command, output_directory):
    ...
    if pr.returncode != 0:
        log.critical(
            "Error executing main processing script.  See output.proc.data for more info"
        )
        return pr.returncode
...
def cleanup_afni_output(output_directory):
    ...
    try:
        ...
        log.warning(
            "Unable to pack standalone QC html report. \
    ...
    try:
        ...
        log.warning(
            "Unable to generate QC archive.  Please check the results to ensure \
