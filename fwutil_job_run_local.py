...
#       [4] - (Optional) Download Output as well.
...
def build_local_test(job, test_path_root, api_key, download_outputs):
    '''
    ...
    # Always grab the job log
    # in the form of {job_id}_logs.txt
    logs = job.get_logs()
    log_fl = open(os.path.join(test_path,job.id+'_logs.txt'),'w')
    for log_line in logs:
        log_fl.write(log_line['msg'])
    log_fl.close()

    #Download Outputs
    if download_outputs:
        dest = fw.get(job.destination.id)
        for fl in dest.files:
            dest.download_file(fl.name, os.path.join(output_dir, fl.name))
...
if __name__=='__main__':
    ...
    Positional inputs:
        ...
        [2] - Job ID
        [3] - (Optional) Directory to save job contents. Defaults to cwd.
        [4] - (Optional) Download outputs as well.
    """
    ...
    build_local_test(job, test_path_root, api_key, download_outputs)
